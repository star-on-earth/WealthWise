 ...
 * Tracker.jsx — v4
 * Changes:
 *  • Added "Family" expense category
 *  • Expenditure-based tax deductions panel (80D, 80C hints)
 *  • computeSavingsFromTracker() exported via localStorage for planner
 */
...
const EXPENSE_CATS = [
  ...
  { key: 'family',     label: 'Family',              color: '#f472b6', icon: '👨‍👩‍👧‍👦' },  // ← NEW
...
// Tax deduction mapping from expense categories
const DEDUCTION_MAP = {
  health:    { section: '80D', label: 'Health Insurance / Medical',  pct: 1.0 },
  insurance: { section: '80D', label: 'Insurance Premiums (health)',  pct: 0.5 },
  investment:{ section: '80C', label: 'Investment instruments',       pct: 1.0 },
  education: { section: '80E', label: 'Education loan interest',      pct: 0.3 },
};
...
const S = {
  ...
  btn:     { background: 'linear-gradient(135deg,var(--gold),var(--goldDim))', color: '#000', border: 'none', borderRadius: 10, padding: '11px 22px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-display)' },
  btnRed:  { background: 'rgba(232,64,64,.15)', color: 'var(--red)', border: '1px solid rgba(232,64,64,.3)', borderRadius: 8, padding: '4px 10px', fontSize: 12, cursor: 'pointer' },
  tabRow:  { display: 'flex', gap: 8, marginBottom: 16 },
  tab:     a => ({ padding: '7px 16px', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-body)', border: a ? '1px solid var(--gold)' : '1px solid var(--border)', background: a ? 'rgba(232,146,26,.12)' : 'var(--bg3)', color: a ? 'var(--gold)' : 'var(--muted)' }),
  ...
  aiBox:   { background: 'linear-gradient(135deg,rgba(29,184,115,.07),rgba(232,146,26,.04))', border: '1px solid rgba(29,184,115,.2)', borderRadius: 12, padding: 18 },
  aiBtn:   { background: 'transparent', color: 'var(--emerald)', border: '1px solid var(--emerald)', borderRadius: 10, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
  tip:     { background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', marginTop: 12, fontSize: 14, color: 'var(--text)', lineHeight: 1.75 },
  dedBox:  { background: 'rgba(29,184,115,.06)', border: '1px solid rgba(29,184,115,.2)', borderRadius: 12, padding: '16px 18px', marginBottom: 16 },
  dedRow:  { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 13 },
};
...
export default function Tracker() {
  ...
  useEffect(() => {
    (async () => {
      if (user) { setTxs(await loadTransactions(user.uid)); }
      else      { setTxs(lsLoad()); }
    })();
  }, [user]);
  ...
  const addTx = async () => {
    ...
    const tx = { amount: +form.amount, category: form.category, note: form.note, date: form.date, type, id: Date.now().toString() };
    if (user) { const ref = await addTransaction(user.uid, tx); setTxs(prev => [{ ...tx, id: ref.id }, ...prev]); }
    else       { const updated = [tx, ...txs]; setTxs(updated); lsSave(updated); }
  ...
  const filtered = useMemo(() => {
    ...
    return txs.filter(t => { const d = new Date(t.date); return d.getMonth() === month && d.getFullYear() === year; });
  }, [txs, view, month, year]);
  ...
  const expBreakdown = useMemo(() => {
    ...
    return Object.entries(map).map(([k,v]) => ({ name: catMeta[k]?.label||k, value: v, color: catMeta[k]?.color||'#888', key: k })).sort((a,b) => b.value - a.value);
  }, [filtered]);

  // ── Deductions from tracker ───────────────────────────────────────────────
  const trackerDeductions = useMemo(() => {
    const map = {};
    filtered.filter(t => t.type === 'expense').forEach(t => { map[t.category] = (map[t.category] || 0) + t.amount; });
    return Object.entries(DEDUCTION_MAP).map(([catKey, ded]) => ({
      ...ded, catKey,
      spent: Math.round(map[catKey] || 0),
      deductible: Math.round((map[catKey] || 0) * ded.pct),
    })).filter(d => d.spent > 0);
  }, [filtered]);

  const totalDeductible = trackerDeductions.reduce((s, d) => s + d.deductible, 0);
  ...
  const barData = useMemo(() => Array.from({ length: 6 }, (_, i) => {
    ...
    return { label: MONTHS[m], income: mTxs.filter(t=>t.type==='income').reduce((s,t)=>s+t.amount,0)/1000, expenses: mTxs.filter(t=>t.type==='expense').reduce((s,t)=>s+t.amount,0)/1000 };
  }), [txs, month, year]);

  const getAiTip = async () => {
    setAiLoading(true); setAiTip('');
    const top = expBreakdown.slice(0,3).map(e=>`${e.name}: ${fmtINR(e.value)}`).join(', ');
    const q = `My top expenses this month: ${top}. Monthly income: ${fmtINR(income)}, Savings: ${fmtINR(savings)} (${savingsRate}%). Give me 2-3 specific actionable tips to improve my savings rate and suggest which Indian investment instruments I should redirect my savings to. Be brief and practical. FY 2026-27 India context.`;
    try { const data = await aiTaxAdvisor({ question: q }); setAiTip(data.answer); }
  ...
  return (
    <div style={S.wrap}>
      ...
      <p style={S.sub}>
        Log income and expenses. Your health and investment spending auto-feeds into tax deduction calculations.
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
        {[
          ...
          { label: 'Savings Rate', value: `${savingsRate}%`, color: +savingsRate>=30?'#1DB873':+savingsRate>=15?'var(--gold)':'var(--red)' },
        ].map(k => (
      ...
      {/* Tax Deductions from Tracker */}
      {trackerDeductions.length > 0 && (
        <div style={S.dedBox}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 14, fontWeight: 700, color: 'var(--emerald)', marginBottom: 12 }}>
            🧾 Tax Deductions from Your Spending — {view === 'month' ? MONTHS[month] : 'All Time'}
          </div>
          {trackerDeductions.map(d => (
            <div key={d.catKey} style={S.dedRow}>
              <div>
                <span style={{ fontWeight: 600 }}>{d.label}</span>
                <span style={{ color: 'var(--gold)', marginLeft: 8, fontSize: 11, fontWeight: 700 }}>§ {d.section}</span>
              </div>
              <div style={{ textAlign: 'right' }}>
                <span style={{ color: 'var(--muted)', fontSize: 12 }}>{fmtINR(d.spent)} spent → </span>
                <span style={{ color: 'var(--emerald)', fontWeight: 700 }}>{fmtINR(d.deductible)} deductible</span>
              </div>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontWeight: 700, fontSize: 14 }}>
            <span style={{ color: 'var(--text)' }}>Total Potential Deductions</span>
            <span style={{ color: 'var(--emerald)' }}>{fmtINR(totalDeductible)}</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8 }}>
            ↑ These amounts are auto-applied in the Tax Planner (old regime) when you use the Planner tab.
          </div>
        </div>
      )}
            ...
            {expBreakdown.map(e => (
              <div key={e.key} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {e.name}
                  {DEDUCTION_MAP[e.key] && <span style={{ fontSize: 10, color: 'var(--emerald)', fontWeight: 700 }}>§{DEDUCTION_MAP[e.key].section}</span>}
                </span>
      ...
      <div style={S.card}>
        ...
        <div style={S.row}>
          <div><label style={S.label}>Amount (₹)</label><input style={S.input} type="number" placeholder="e.g. 5000" value={form.amount} onChange={set('amount')} onFocus={focusGold} onBlur={blurReset} /></div>
          <div>
            <label style={S.label}>Category</label>
            <select style={S.select} value={form.category} onChange={set('category')}>
              {(type==='expense' ? EXPENSE_CATS : INCOME_CATS).map(c => <option key={c.key} value={c.key}>{c.icon} {c.label}</option>)}
            </select>
          </div>
          <div><label style={S.label}>Date</label><input style={S.input} type="date" value={form.date} onChange={set('date')} onFocus={focusGold} onBlur={blurReset} /></div>
          <div><label style={S.label}>Note (optional)</label><input style={S.input} type="text" placeholder="e.g. Swiggy order" value={form.note} onChange={set('note')} onFocus={focusGold} onBlur={blurReset} /></div>
        </div>
