/**
 * IncomeForm.jsx — v4
 * Multi-source income + HUF + loans + agricultural income
 * Senior citizen auto-detected from age prop (no separate toggle)
 * Expenditure deductions pulled from tracker
 */
...
const S = {
  ...
  srcIcon:   { fontSize: 18, marginBottom: 4 },
  ...
  kpi:       (c) => ({ flex: '1 1 120px', background: `${c}12`, border: `1px solid ${c}30`,
...
export default function IncomeForm({ value = {}, onChange, age = 30, entityType, onEntityChange, loanDeductions = {}, onLoanChange }) {
  ...
  const setLoan = (key, v) => onLoanChange({ ...loanDeductions, [key]: +v || 0 });
  ...
  const taxResult = useMemo(() => {
    ...
    return computeMultiIncomeTax(value, age, entityType, loanDeductions, trackerDeductions);
  }, [value, age, entityType, loanDeductions, trackerDeductions]);

  // Group income sources
  const ordinaryGroup  = INCOME_SOURCES.filter(s => ['salary','business','freelance','rental','fd_interest','savings_int','dividends','other'].includes(s.key));
  const capitalGroup   = INCOME_SOURCES.filter(s => ['ltcg_equity','stcg_equity','ltcg_property'].includes(s.key));
  const specialGroup   = INCOME_SOURCES.filter(s => ['agricultural','crypto'].includes(s.key));

  const SrcGroup = ({ title, badge, badgeColor, sources }) => (
    <div style={S.section}>
      ...
      <div style={S.grid}>
        {sources.map(src => {
          ...
          return (
            <div key={src.key}>
              <div style={S.srcCard(isOn)} onClick={() => toggleSrc(src.key)}>
                <div style={S.srcIcon}>{src.icon}</div>
                <div style={S.srcLabel}>{src.label}</div>
                <div style={S.srcNote}>{src.note}</div>
                {isOn && (
                  <input style={S.input} type="number" placeholder="Annual amount (₹)"
                    value={value[src.key] || ''}
    ...
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={S.entityRow}>
          {ENTITY_TYPES.map(et => (
            <button key={et.key} style={S.entityBtn(entityType === et.key)} onClick={() => onEntityChange(et.key)}>
        ...
        {entityType === 'huf' && (
          <div style={S.infoBox}>
            🏛️ HUF has the same basic exemption (₹2.5L) as an individual. No senior citizen benefit applies to HUF.
            HUF can claim all business deductions and invest in its own name. Interest exemption: ₹50K (same as senior).
          </div>
        )}
        {isSenior && entityType === 'individual' && (
          <div style={{ ...S.infoBox, background: 'rgba(29,184,115,.07)', borderColor: 'rgba(29,184,115,.2)' }}>
            ✅ Senior citizen (60+) detected — 80TTB ₹50K interest exemption auto-applied. Basic exemption ₹3L in old regime.
          </div>
        )}
      </div>

      <SrcGroup title="Ordinary Income"   badge="Slab Rate"     badgeColor="#9B72CF" sources={ordinaryGroup}  />
      <SrcGroup title="Capital Gains"     badge="Special Rates" badgeColor="#4A9EE8" sources={capitalGroup}   />
      <SrcGroup title="Special Income"    badge="Exempt / 30%"  badgeColor="#E84040" sources={specialGroup}   />
      ...
      <div style={S.section}>
        ...
        <div style={S.grid}>
          {LOAN_TYPES.map(loan => {
            ...
            return (
              <div key={loan.key}>
                <div style={S.srcCard(isOn)} onClick={() => toggleLoan(loan.key)}>
      ...
      {/* Tracker-derived deductions notice */}
      {(trackerDeductions.health80D > 0 || trackerDeductions.potential80C > 0) && (
        <div style={S.infoBox}>
          ...
          {trackerDeductions.health80D > 0 && `₹${fmtINR(trackerDeductions.health80D)} health spending → auto-applied as 80D deduction. `}
          {trackerDeductions.potential80C > 0 && `₹${fmtINR(trackerDeductions.potential80C)} investment spending → verify against 80C instruments.`}
        </div>
      )}
      ...
      {taxResult && (
        <div style={{ marginTop: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase',
                        letterSpacing: '0.5px', marginBottom: 12 }}>Live Tax Preview</div>
          ...
          <div style={S.kpiRow}>
            {[
              ...
              { label: 'Marginal Slab Rate',  value: `${(taxResult.marginalSlabRate * 100).toFixed(0)}%`,         color: '#9B72CF'        },
            ].map(k => (
          ...
          {taxResult.specialTaxTotal > 0 && (
            <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '14px 16px', marginTop: 14 }}>
              ...
              {taxResult.cryptoTax > 0        && <div style={{ ...S.taxRow, borderBottom: 'none' }}><span>Crypto/VDA 30% flat (no deductions)</span><span style={{ color: '#E84040', fontWeight: 700 }}>{fmtINR(taxResult.cryptoTax)}</span></div>}
            </div>
          )}

          {/* Regime comparison showing DIFFERENT slab rates */}
            ...
            {[
              ...
              <div key={r.key} style={S.regCard(taxResult.bestRegime === r.key)}>
                ...
                <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                  Marginal slab: <strong style={{ color: 'var(--text)' }}>{(r.slab * 100).toFixed(0)}%</strong>
                </div>
          ...
          {taxResult.agriIncome > 0 && (
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>
              🌾 Agricultural income ₹{fmtINR(taxResult.agriIncome)} is exempt but used for rate computation (partial integration method).
            </div>
          )}
